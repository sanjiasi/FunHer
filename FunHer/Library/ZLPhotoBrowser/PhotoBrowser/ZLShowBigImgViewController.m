...
- (void)viewDidLayoutSubviews
{
    ...
    _btnDone.frame = CGRectMake(frame.size.width-92-inset.right, 7, 80, 30);
}
...
- (void)initNavView
{
    ...
    _navView.backgroundColor = configuration.navBarColor;//[configuration.navBarColor colorWithAlphaComponent:.9];
    [self.view addSubview:_navView];
    
    _btnBack = [UIButton buttonWithType:UIButtonTypeCustom];
    [_btnBack setImage:[UIImage imageNamed:@"backItem"] forState:UIControlStateNormal];//GetImageWithName(@"zl_navBack")
    ...
    UIImage *normalImg = [UIImage imageNamed:@"uncheckState"];//GetImageWithName(@"zl_btn_circle");
...
- (void)initBottomView
{
    ...
    [_btnDone setTitle:@"Import" forState:UIControlStateNormal];
    ...
    _btnDone.frame = CGRectMake(kViewWidth - 92, 7, 80, 30);
...
- (void)resetDontBtnState
{
    ...
    if (nav.arrSelectedModels.count > 0) {
        ...
        [_btnDone setTitle:[NSString stringWithFormat:@"%@(%ld)", @"Import", nav.arrSelectedModels.count] forState:UIControlStateNormal];
    } else {
        [_btnDone setTitle:@"Import" forState:UIControlStateNormal];
    }
}
