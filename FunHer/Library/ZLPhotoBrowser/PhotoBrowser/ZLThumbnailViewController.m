...
//        inset = self.view.safeAreaInsets;
        inset = UIEdgeInsetsMake(kNavBarAndStatusBarHeight, 0, kBottomSafeHeight, 0);
    }
    ...
    self.collectionView.frame = CGRectMake(inset.left, 0, width, kViewHeight-inset.bottom-bottomViewH-inset.top);
    
    if (!showBottomView) return;
    
    self.bottomView.frame = CGRectMake(inset.left, kViewHeight-bottomViewH-inset.bottom-inset.top, width, bottomViewH+inset.bottom);
...
//    self.btnPreView.frame = CGRectMake(offsetX, 7, GetMatchValue(GetLocalLanguageTextValue(ZLPhotoBrowserPreviewText), 15, YES, bottomBtnH), bottomBtnH);
    self.btnPreView.frame = CGRectMake(offsetX, 0, bottomViewH, bottomViewH);
    offsetX = CGRectGetMaxX(self.btnPreView.frame) + 10;
    
    self.filterSetBtn.frame = CGRectMake((kScreenWidth - bottomViewH)/2.0, 0, bottomViewH, bottomViewH);
...
- (void)resetBottomBtnsStatus:(BOOL)getBytes
{
    ...
    if (nav.arrSelectedModels.count > 0) {
        ...
        [self.btnDone setTitle:[NSString stringWithFormat:@"%@(%ld)", @"Import", nav.arrSelectedModels.count] forState:UIControlStateNormal];
        ...
        [self.btnPreView setTitleColor:configuration.bottomBtnsDisableTitleColor forState:UIControlStateNormal];
        ...
        self.labPhotosBytes.text = nil;
        [self.btnDone setTitle:@"Import" forState:UIControlStateDisabled];
    ...
    if (nav.arrSelectedModels.count >= 2) {
        self.filterSetBtn.hidden = NO;
        self.btnPreView.hidden = NO;
    } else {
        self.filterSetBtn.hidden = YES;
...
//    [self.btnPreView setTitle:GetLocalLanguageTextValue(ZLPhotoBrowserPreviewText) forState:UIControlStateNormal];
    [self.btnPreView setImage:[UIImage imageNamed:@"crop_auto"] forState:UIControlStateNormal];
    [self.btnPreView addTarget:self action:@selector(cropAuto_Click:) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.filterSetBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.filterSetBtn setImage:[UIImage imageNamed:@"filter_batch"] forState:UIControlStateNormal];
    [self.filterSetBtn addTarget:self action:@selector(btnPreview_Click:) forControlEvents:UIControlEventTouchUpInside];
    [self.bottomView addSubview:self.filterSetBtn];
    self.filterSetBtn.hidden = YES;
    
    self.btnDone = [UIButton buttonWithType:UIButtonTypeCustom];
    self.btnDone.titleLabel.font = [UIFont systemFontOfSize:15];
    self.btnDone.titleLabel.numberOfLines = 1;
    self.btnDone.titleLabel.adjustsFontSizeToFitWidth = YES;
    [self.btnDone setTitle:@"Import"forState:UIControlStateNormal];//GetLocalLanguageTextValue(ZLPhotoBrowserDoneText)
...
//    UIBarButtonItem *backItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"close_navItem"] style:UIBarButtonItemStylePlain target:self action:@selector(navRightBtn_Click)];//GetImageWithName(@"zl_navBack")
    UIButton *backItem = [UIButton buttonWithType:UIButtonTypeCustom];
    backItem.frame = CGRectMake(0, 0, 44, 44);
    ...
    self.navigationItem.titleView = self.btnShowPhotos;
...
//    self.showFilterLab.frame = CGRectMake((kScreenWidth-getWidth)/2, (kScreenHeight-45)/2, getWidth, 45);
...
- (void)layoutChange:(UIButton *)sender {
    sender.selected = !sender.selected;
    self.columnLayout = sender.selected ? 4 : 3;
    CGFloat width = MIN(kViewWidth, kViewHeight);
...
- (void)showPhotoLibrary:(UIButton *)sender {
    sender.selected = !sender.selected;
    [self popPhotoLibraryView];
}

- (void)popPhotoLibraryView {
    [self takeLibraryShow];
}

- (void)takeLibraryShow {
    SSPopListView *listView = [[SSPopListView alloc] initWithItems:self.albumArt selectedBlock:^(NSInteger index) {
        if (index < self.albumArt.count) {
            ZLAlbumListModel *model = self.albumArt[index];
            self.albumListModel = model;
            self.btnShowPhotos.selected = NO;
            [self.btnShowPhotos setTitle:self.albumListModel.title forState:UIControlStateNormal];
            ZLImageNavigationController *nav = (ZLImageNavigationController *)self.navigationController;
